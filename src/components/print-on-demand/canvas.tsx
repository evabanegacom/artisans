...
const CanvasApp: React.FC<CanvasAppProps> = ({ position = [0, 0, 5], fov = 35, preferred, fill }) => {
  const canvasApp = useMemo(() => (
    <div id="canvas-container" style={{ width: '100%', height: '100vh', overflow: 'hidden' }}>
      <Canvas shadows={false} camera={{ position, fov }} gl={{ preserveDrawingBuffer: true }} eventSource={document.getElementById('canvas-container') as any} eventPrefix="client">
...
const Backdrop: React.FC = () => {
  ...
  return (
    <AccumulativeShadows ref={shadows} temporal frames={30} alphaTest={0.85} scale={10} rotation={[Math.PI / 2, 0, 0]} position={[0, 0, -0.14]}>
      <RandomizedLight amount={2} radius={9} intensity={0.35} ambient={0.25} position={[5, 5, -10]} />
      <RandomizedLight amount={2} radius={5} intensity={0.15} ambient={0.55} position={[-5, 5, -9]} />
    </AccumulativeShadows>
  );
};
...
const Shirt: React.FC<ShirtProps> = ({ preferred, fill }) => {
  const snap = useSnapshot(state);
  const texture = useTexture(preferred || '/react.png');
  ...
  return (
    <mesh castShadow geometry={nodes.T_Shirt_male.geometry} material={materials.lambert1} material-roughness={1} scale={[3.0, 3.0, 3.0]} dispose={null}>
      {!fill && <Decal position={[0, 0.04, 0.15]} rotation={[0, 0, 0]} scale={0.20} map={texture as any} />}
    </mesh>
  );
};
