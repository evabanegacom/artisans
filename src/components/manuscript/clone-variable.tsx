...
const CloneVariable: React.FC<RespecifyDatasetProps> = ({ variables }) => {
  ...
  const handleChangeVariableType = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit}>
        <div className='text-sm font-medium text-[#1E293B]'>Select Variable to Clone</div>
        <select
        ...
        {selectedVariable &&
        <>
        <div className='mt-5 text-sm font-medium text-[#1E293B]'>Enter Name of Newly Cloned Variable</div>
        <input
          className='border w-full border-gray-300 rounded-lg mt-2 p-3 text-sm font-normal shadow-md outline-none text-[#1E293B]'
